...
import type { NoteType, NoteTier, NoteConfidence, NoteStatus } from '../types.js'
import {
  VALID_NOTE_TYPES,
  VALID_NOTE_TIERS,
...
export const addCommand = new Command('add')
  ...
  .action(async (file, opts) => {
    if (opts.type && !VALID_NOTE_TYPES.includes(opts.type as NoteType)) {
      console.error(`Error: invalid type "${opts.type}". Valid types: ${VALID_NOTE_TYPES.join(', ')}`)
      process.exitCode = 1
      return
    }

    if (opts.tier && !VALID_NOTE_TIERS.includes(opts.tier as NoteTier)) {
      console.error(`Error: invalid tier "${opts.tier}". Valid tiers: ${VALID_NOTE_TIERS.join(', ')}`)
      process.exitCode = 1
      return
    }
    ...
    if (opts.reviewInterval && !/^\d+[dwm]$/.test(opts.reviewInterval)) {
      console.error(`Error: invalid review-interval "${opts.reviewInterval}". Use format like 30d, 4w, 3m`)
      process.exitCode = 1
      return
    }
