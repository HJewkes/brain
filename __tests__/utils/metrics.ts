...
export function mrr(allResults: Array<{ retrieved: string[]; relevant: Set<string> }>): number {
...
export function aggregateMetrics(
  ...
  const avgHitRate =
    allResults.reduce((sum, { retrieved: r, relevant: rel }) => sum + hitRate(r, rel), 0) / n;
  const avgMrr = mrr(allResults);
  const avgPrecision =
    allResults.reduce((sum, { retrieved: r, relevant: rel }) => sum + precisionAtK(r, rel, k), 0) /
    n;
  const avgRecall =
    allResults.reduce((sum, { retrieved: r, relevant: rel }) => sum + recallAtK(r, rel, k), 0) / n;
  const avgNdcg =
    allResults.reduce((sum, { retrieved: r, relevant: rel }) => sum + ndcgAtK(r, rel, k), 0) / n;
