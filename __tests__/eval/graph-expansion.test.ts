...
describe('graph expansion', () => {
  ...
  describe('VBT cluster traversal', () => {
    test('direct relations from VBT overview pull related notes', () => {
      ...
      const vbtNotes = corpus.notes.filter((n) =>
        n.id.includes('vbt') || n.id.includes('velocity') || n.id.includes('rpe'),
      )
  ...
  describe('score decay', () => {
    test('depth-1 nodes score higher than depth-2 nodes', () => {
      const vbtNotes = corpus.notes.filter((n) =>
        n.id.includes('vbt') || n.id.includes('velocity'),
      )
      ...
      if (depth1.length > 0 && depth2.length > 0) {
        ...
        const expanded = expandResults(env.db, [centralId], 2)
        const d1Scores = expanded.filter((e) =>
          depth1.some((n) => n.id === e.noteId),
        )
        const d2Scores = expanded.filter((e) =>
          depth2.some((n) => n.id === e.noteId),
        )
  ...
  describe('edge cases', () => {
    ...
    test('circular relations do not infinite loop', () => {
      ...
      const noteWithRelations = corpus.notes.find((n) =>
        n.frontmatter.related && n.frontmatter.related.length > 0,
      )
