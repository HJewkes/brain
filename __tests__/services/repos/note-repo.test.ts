...
describe('NoteRepo', () => {
  ...
  describe('metadata filtering', () => {
    ...
    it('getFilteredNoteIdsByMetadata intersects with baseIds', () => {
      ...
      const result = db.getFilteredNoteIdsByMetadata(
        [{ field: 'category', value: 'context-management' }],
        baseIds
      );
      expect(result.has('n1')).toBe(true);
      expect(result.has('n2')).toBe(false);
  ...
  describe('facet counts', () => {
    it('getFacetCounts returns value counts for scalar field', () => {
      ...
      expect(facets).toEqual([
        { value: 'deterministic', count: 2 },
        { value: 'structural', count: 1 },
      ]);
    });

    it('getFacetCounts handles array fields', () => {
      db.upsertNote(makeNote({ id: 'n1', metadata: JSON.stringify({ layers: [1, 3] }) }));
      db.upsertNote(makeNote({ id: 'n2', metadata: JSON.stringify({ layers: [1, 2] }) }));

      const facets = db.getFacetCounts('layers', new Set(['n1', 'n2']));
      expect(facets).toEqual([
        { value: '1', count: 2 },
        { value: '2', count: 1 },
        { value: '3', count: 1 },
    ...
    it('getFacetCounts skips notes with null metadata', () => {
      db.upsertNote(makeNote({ id: 'n1', metadata: JSON.stringify({ strength: 'high' }) }));
      db.upsertNote(makeNote({ id: 'n2', metadata: null }));

      const facets = db.getFacetCounts('strength', new Set(['n1', 'n2']));
      expect(facets).toEqual([{ value: 'high', count: 1 }]);
    });
  });
