...
import type { NoteRecord, Chunk, Embedder } from '../../src/types.js'

function tmpDbPath(): string {
  return join(tmpdir(), `brain-search-test-${randomUUID()}.db`)
}
...
function seedTestNotes(db: BrainDB): void {
  ...
  const chunkContents: Record<string, { heading: string; content: string }[]> = {
    'ts-patterns': [
      { heading: 'Factory Pattern', content: 'The factory pattern in TypeScript creates objects without specifying exact classes. Use generics for type-safe factories.' },
      { heading: 'Observer Pattern', content: 'Observer pattern implements event-driven architecture. TypeScript interfaces make observers type-safe.' },
    ],
    'ble-protocol': [
      { heading: 'BLE Basics', content: 'Bluetooth Low Energy uses GATT profiles for communication. Services contain characteristics for data exchange.' },
      { heading: 'Device Connection', content: 'BLE devices advertise services. Central devices scan and connect using device UUIDs and service characteristics.' },
    ],
    'react-hooks': [
      { heading: 'useState', content: 'useState hook manages local component state. Use TypeScript generics for typed state values.' },
      { heading: 'useEffect', content: 'useEffect handles side effects and cleanup. Dependencies array controls re-execution.' },
    ],
    'workout-algo': [
      { heading: 'Velocity Calculations', content: 'Mean velocity computed from position sensor data. Peak velocity uses derivative of displacement over time.' },
      { heading: 'Load-Velocity Profile', content: 'Linear regression of load vs velocity creates athlete profile. Predicts one-rep max from submaximal efforts.' },
    ],
    'css-grid': [
      { heading: 'Grid Basics', content: 'CSS Grid provides two-dimensional layout control. grid-template-columns and grid-template-rows define structure.' },
      { heading: 'Grid Areas', content: 'Named grid areas simplify complex layouts. grid-template-areas provides visual layout definition.' },
    ],
  }
...
describe('search service', () => {
  ...
  describe('empty and edge cases', () => {
    ...
    it('returns BM25-only results when no vectors are indexed', async () => {
      ...
      freshDb.upsertNoteFTS('fts-only', 'FTS Only Note', 'A note with only FTS indexing', 'This note about TypeScript has only FTS content and no vectors')
